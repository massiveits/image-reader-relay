...
1. **Stashes the image in memory** (never writes to disk) and replaces the attachment with a note.
2. Registers a **`read_image` tool** the main model can call with its own question.
3. Relays the image to a configured **vision model** in a throwaway session, cleans up, and returns the answer as a tool result.
