...
const ImageReaderRelay = async ({ client }, rawOptions) => {
  ...
  const modelSupportsImages = async (model) => {
    ...
    const checkCaps = (target) => {
      if (!target || typeof target !== "object") return false
      const caps = target.capabilities ?? target
      const input = caps.input ?? target.input
      const modalities = caps.modalities?.input ?? target.modalities?.input
      return !!(
        caps.attachment ||
        target.attachment ||
        input?.image ||
    ...
    try {
      ...
      for (const provider of providers) {
        ...
        const found = Array.isArray(modelCatalog)
          ? modelCatalog.find((m) => m?.id === model.modelID || m?.name === model.modelID)
          : (modelCatalog[model.modelID] ?? Object.values(modelCatalog).find((m) => m?.id === model.modelID || m?.name === model.modelID))

        if (found) {
          const supports = checkCaps(found)
